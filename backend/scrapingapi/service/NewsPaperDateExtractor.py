...
class NewsPaperDateExtractor:
    ...
    def extract_date_bdnews24(url):
        ...
        if response.status_code == 200:
            ...
            if date_div:
                ...
                date_formats = ['%d %B %Y, %I:%M %p', '%d %b %Y, %I:%M %p']  # List of date formats to try
    ...
    def extract_date_bangladeshpost(url):
        ...
        if response.status_code == 200:
            ...
            if date_info:
                date_str = re.search(r'Published\s*:\s*(\d{1,2} \w{3} \d{4} \d{1,2}:\d{2} [APM]{2})',
                                     date_info.get_text())
                if date_str:
    ...
    def extract_date_dailystar(url):
        ...
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            date_div = soup.find('div', class_='date')  # Update class based on actual HTML structure
            if date_div:
                ...
                if match:
                    ...
                    match = re.search(r'Last update on: (\w+ \w+ \d+, \d{4} \d{2}:\d{2} [APM]{2})', datetime_str)
                    if match:
                        datetime_str = match.group(1)
                    else:
                        return None
    ...
    def extract_date_ittefaq(url):
        ...
        if response.status_code == 200:
            ...
            if date_span:
                date_str = date_span.get('content')  # Get the 'content' attribute which has the date and time
