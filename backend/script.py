...
for entry in data:
    ...
    accident = float(entry['number_of_accidents_occured']) if entry['number_of_accidents_occured'] else 0.0

    # Convert killed count to float
    killed = float(entry['total_number_of_people_killed']) if entry['total_number_of_people_killed'] else 0.0

    # Convert injured count to float
    injured = float(entry['total_number_of_people_injured']) if entry['total_number_of_people_injured'] else 0.0

    vehicles = [entry['primary_vehicle_involved'], entry['secondary_vehicle_involved'],
                entry['tertiary_vehicle_involved']], entry['any_more_vehicles_involved']
    ...
    if accident_date.year == current_date.year:
        ...
        if is_last_30_days_of_year(accident_date):
            daily_key = accident_date.strftime('%Y-%m-%d')
            daily_totals_last_30_days[daily_key] += killed
...
most_frequent_locations_per_year = {year: locations.most_common(1)[0][0] if locations else None
...
def upsert_data(daily_totals, monthly_totals, yearly_totals, daily_injured_totals, monthly_injured_totals,
                yearly_injured_totals, yearly_accident_totals, most_frequent_locations_per_year, yearly_vehicle_accident_count):
    conn = None
    cursor = None
    try:
        conn = mysql.connector.connect(**db_config)
        ...
        for year, total_killed in yearly_totals.items():
            ...
            daily_deaths_json = json.dumps({k: v for k, v in daily_totals.items() if k.startswith(str(year))})
            # Prepare the daily injured JSON
            daily_injured_json = json.dumps({k: v for k, v in daily_injured_totals.items() if k.startswith(str(year))})
            ...
            monthly_injured_json = json.dumps(
                {f"{k[0]}-{k[1]:02d}": v for k, v in monthly_injured_totals.items() if k[0] == year})
...
upsert_data(daily_totals_last_30_days, monthly_totals, yearly_totals, daily_injured_totals_last_30_days,
            monthly_injured_totals, yearly_injured_totals, yearly_accident_totals, most_frequent_locations_per_year,
